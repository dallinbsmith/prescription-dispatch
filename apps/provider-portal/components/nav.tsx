    ...
    <>
      {navItems.map((item) => (
        <Link
          ...
          className={cn(
            "text-sm font-medium transition-colors hover:text-doctor-600",
            isActive(item.href)
              ? "text-doctor-600 font-semibold"
              : "text-neutral-600",
            mobile && "block py-2"
          )}
        >
  ...
  return (
    <header className="border-b border-neutral-200 bg-white">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        <Link href="/" className="text-xl font-bold text-doctor-600">
          Prescription Dispatch Provider
        </Link>
